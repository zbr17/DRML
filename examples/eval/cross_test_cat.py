...
for idx, (train_split, test_split) in enumerate(KFold(n_splits=n_split).split(total_index)):
    ...
    with torch.no_grad():
        for k, v in tester.datasets.items():
            ...
            output_embeddings.append(tester.embeddings)
            output_labels.append(tester.labels)

embeddings = np.concatenate(output_embeddings, axis=1)
...
save_f = open("/home/zbr/code/relational_dml/results/{}-cat.txt".format(save_name), mode="w")
output_list = []
